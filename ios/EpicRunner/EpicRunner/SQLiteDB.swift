...
//		println("SQLiteDB - Init method")
        ...
        // ** EDIT START ** //
        let virginDb = !(fm.fileExistsAtPath(path));
        println(path);
        if virginDb {
            // Create new datebase
        ...
        // ** EDIT START ** //
        if virginDb {
            execute("CREATE TABLE IF NOT EXISTS settings (loggedInUserId INTEGER, loggedInUserName TEXT, loggedInLevel INTEGER NOT NULL DEFAULT 0, loggedInTuRunSelector BOOL NOT NULL DEFAULT 0, loggedInSessionToken TEXT)");
            ...
            execute("CREATE TABLE stat_screen (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL UNIQUE, statSessionId INTEGER NOT NULL, screen TEXT, enteredTimestamp INTEGER, exitedTimestamp INTEGER)");
